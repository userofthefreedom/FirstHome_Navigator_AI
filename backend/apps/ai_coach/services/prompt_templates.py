    ...
    *,
    ...
    if any(keyword in normalized for keyword in ["부족", "계약금", "돈", "자금", "얼마"]):
        ...
        actions = [
            ...
            "당첨자 발표 뒤 서류 제출 기간을 함께 확인하세요.",
        ]
    elif any(keyword in normalized for keyword in ["pdf", "공고문", "서류", "확인"]):
        reply = _document_reply(notice, plan)
        actions = [
            "공식 공고문 PDF의 주택가격표와 납부조건 표를 확인하세요.",
            "무주택, 소득, 자산, 청약통장 기준을 별도로 체크하세요.",
            "화면의 분석 결과에 확인 필요 표시가 있으면 원문을 우선하세요.",
        ]
    else:
            ...
            messages=[
                {
                    "role": "system",
                    "content": (
                        "너는 FirstHome Navigator AI의 청약 준비 코치다. "
                        "답변은 한국어 2~3문장, 350자 이내로 작성한다. "
                        "금액과 날짜가 있으면 먼저 말하고, 불필요한 일반론·번호 목록·마크다운 굵게 표시를 쓰지 않는다. "
                        "당첨, 신청 가능, 자격 충족, 대출 승인 같은 확정 표현은 금지한다. "
                        "반드시 '공식 공고문 확인이 필요합니다'라는 취지의 문장을 포함한다."
                    ),
                },
...
def _funding_reply(notice: dict[str, Any], plan: dict[str, Any]) -> str:
    ...
    return (
        ...
        f"현재 준비 가능 현금 {plan['available_cash']:,}원을 반영하면 부족액은 약 {plan['shortfall']:,}원이고, "
...
def _schedule_reply(notice: dict[str, Any], plan: dict[str, Any]) -> str:
    ...
    if first_payment:
        return (
            f"{notice['title']}의 첫 납부 항목은 {first_payment['label']}이며 "
            f"금액은 약 {int(first_payment['amount']):,}원입니다. 날짜는 {first_payment.get('date') or '공식 확인 필요'}로 표시되어 있습니다. "
            "중도금 일정은 공정이나 기관 안내에 따라 달라질 수 있어 공식 공고문을 함께 확인해야 합니다."
        )
    return (
...
def _document_reply(notice: dict[str, Any], plan: dict[str, Any]) -> str:
    ...
    return (
        ...
        "무주택·소득·자산·청약통장 기준을 우선 확인하세요. 분석 결과는 참고용이며 원문과 다르면 원문이 우선입니다."
    )
...
def _general_reply(notice: dict[str, Any], plan: dict[str, Any]) -> str:
    return (
        ...
        f"현재 자금 로드맵상 부족액은 약 {plan['shortfall']:,}원이며, "
        "추천 점수와 AI 답변은 참고용입니다. 신청 가능 여부와 납부 조건은 공식 공고문에서 확인해야 합니다."
    )
...
def _chat_context(notice: dict[str, Any], plan: dict[str, Any], profile: dict[str, Any]) -> str:
    ...
    return (
        ...
        f"선택옵션: {plan.get('unit_type', '공고 대표값')} {plan.get('floor_group', '')}\n"
