...
현재 버전은 **Django REST Framework 백엔드 + Vue 3 프론트엔드** 구조입니다. 운영/개발의 기본 방향은 실제 API 데이터를 우선 사용하고, `backend/fixtures/firsthome_mvp.json` fixture는 실제 LH 수집이 실패하거나 소유형 공고가 너무 적을 때 시연 흐름을 보조하는 안전망으로 사용합니다.
...
- 실제 LH 소유형 공고가 부족할 때 fixture 보조 fallback
...
# 실제 LH 소유형 공고가 부족할 때 fixture를 보조로 붙이는 기준
...
기본 실행은 DB가 비어 있어도 fixture fallback으로 동작합니다. 발표 직전처럼 고정된 데모 상태가 필요할 때만 아래 명령으로 fixture를 DB에 직접 적재합니다.
...
실제 LH 공고가 DB에 들어오면 DB 데이터가 먼저 노출됩니다. 단, 서비스 대상 소유형 공고가 너무 적으면 기본 설정상 fixture 서비스 대상 공고가 뒤에 보충됩니다. 기준값은 `FIRSTHOME_MIN_SERVICE_NOTICES`, 보충 사용 여부는 `FIRSTHOME_ENABLE_FIXTURE_SUPPLEMENT`로 조정할 수 있습니다.
...
공공데이터포털 서비스 키를 `backend/.env`의 `DATA_GO_KR_SERVICE_KEY`에 넣으면 LH 분양임대공고 목록을 DB로 수집할 수 있습니다. 다만 FirstHome의 핵심 범위인 소유형 공공분양/신혼희망타운/민간참여형 공공분양은 LH 최신 목록에서 항상 앞쪽에 나오지 않습니다. 최신 1~3페이지만 가져오면 임대형 공고만 들어오거나 추천 대상이 거의 없을 수 있으므로, 실제 데이터 테스트 때는 소유형 후보를 우선 보강하는 옵션을 함께 사용합니다.

```bash
cd backend

# 소유형 공공분양 후보가 잡히는지 넓게 확인하고 DB 변경 없음
python manage.py import_lh --dry-run --service-target-only --page-size 100 --max-pages 5 --with-supply-info --supply-limit 30

# 소유형 공공분양 후보만 DB에 저장하고 공급정보로 대표 분양가/주택형 옵션을 보강
python manage.py import_lh --service-target-only --page-size 100 --max-pages 5 --with-supply-info --supply-limit 30

# 실제 PDF 분석까지 실행해 납부 일정과 근거 문장을 보강
...
# fixture 공고를 지우고 실제 LH 소유형 공고만 다시 저장
...
- `--clear`를 붙이면 기존 DB 공고가 지워집니다. 그래도 DB가 비거나 소유형 공고가 부족하면 화면은 fixture 보조 fallback으로 완전히 비지 않게 동작합니다.
- 발표용 고정 데이터를 DB에 직접 넣어야 할 때만 `load_firsthome_fixture --with-demo-analysis`를 사용하세요.
- `--service-target-only`를 붙이면 임대형/토지/상가/오피스텔 등 서비스 범위 밖 공고는 DB에 저장하지 않습니다. 잔여세대·선착순·동호지정 공고라도 공공분양 소유형이면 서비스 대상에 포함합니다.
- `--max-pages`가 너무 작으면 소유형 공공분양을 놓칠 수 있습니다. 현재 개발 기준은 5페이지부터 확인하고, 데이터가 부족하면 더 늘립니다.
- `--with-supply-info`는 소유형 후보를 우선 조회해 대표 분양가와 주택형별 임시 옵션을 만듭니다. 정확한 계약금/중도금 회차는 공식 PDF 분석 결과가 우선입니다.
- 중도금은 공고마다 0회, 1회, 여러 회차로 달라질 수 있습니다. 서비스는 중도금을 고정 항목으로 보지 않고 `PaymentSchedule`의 반복 일정으로 저장하며, 공식 PDF 분석이 성공하면 회차별 금액과 일정을 그대로 표시합니다. 신혼희망타운 잔여세대처럼 `입주잔금 + 융자금 + 할부금` 구조인 공고는 할부금을 `installment_payment`로 별도 표시합니다.
- 같은 공고 안에 사전청약 당첨자와 일반 당첨자의 납부 조건이 함께 있으면 주택형 옵션을 `pre_subscription`, `general_supply` 납부안으로 나누어 저장합니다.
- LH 목록 API에는 가격, 계약일, 세부 자격이 빠진 공고가 많습니다. 가격이 확인되지 않는 공고는 서비스에서 자금 계산을 추정하지 않고 “공식 공고 확인 필요”로 표시합니다.
- `--with-supply-info`를 쓰려면 공공데이터포털에서 LH 분양임대공고별 공급정보 조회 서비스도 활용승인되어 있어야 합니다.
- 기본 fallback 기준은 소유형 서비스 대상 공고 3건입니다. 기준을 바꾸려면 `backend/.env`에 `FIRSTHOME_MIN_SERVICE_NOTICES=5`처럼 지정합니다. fixture 보충을 끄려면 `FIRSTHOME_ENABLE_FIXTURE_SUPPLEMENT=false`를 지정합니다.
...
# 1. 기본 DB 준비
# DB가 비어 있어도 fixture fallback이 동작하므로 처음부터 fixture를 적재할 필요는 없습니다.
# 발표용 고정 데이터가 필요할 때만 다음 명령을 사용하세요.
# python manage.py load_firsthome_fixture --with-demo-analysis
...
# 6. LH 소유형 공공분양 후보와 공급정보 연결 확인
python manage.py import_lh --dry-run --service-target-only --page-size 100 --max-pages 5 --with-supply-info --supply-limit 30

# 7. 실제 LH 소유형 공고 및 주택형/분양가 보강 저장
python manage.py import_lh --service-target-only --page-size 100 --max-pages 5 --with-supply-info --supply-limit 30

# 8. 공식 PDF 분석으로 납부 일정과 근거 문장 보강
python manage.py analyze_notice_documents --provider=LH --limit 20 --report-json=reports/lh_actual_readiness.json --report-md=reports/lh_actual_readiness.md
```
...
python manage.py import_lh --service-target-only --page-size 100 --max-pages 5 --with-supply-info --supply-limit 30
python manage.py analyze_notice_documents --provider=LH --limit 20 --report-json=reports/lh_actual_readiness_review.json
...
### 16-6. 실제 API를 수집했는데 화면에 fixture가 함께 보임

DB에 실제 데이터가 들어갔는지 먼저 확인하세요. 현재 서비스는 DB 데이터를 우선 사용하되, 소유형 서비스 대상 공고가 너무 적으면 fixture 공고를 뒤에 보충합니다. 추천 카드의 `data_source`가 `LH API`이면 실제 수집 공고이고, `fixture fallback`이면 시연 안정성을 위한 보조 데이터입니다.
...
fixture 보충을 끄고 실제 DB 결과만 확인하려면 `backend/.env`에 아래 값을 넣고 Django 서버를 재시작합니다.
