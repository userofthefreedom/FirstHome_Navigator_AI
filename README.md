...
| 이름 | 역할 | 담당 영역 |
|---|---|---|
| 오세진 | 팀장 | 서비스 기획, 백엔드 설계 및 구현, 추천/자금/AI 규칙 설계 |
| 박성연 | 팀원 | 프론트엔드 화면 구현, 사용자 경험 개선, 발표자료 제작 |
| 전희창 | 팀원 | QA, 시연 시나리오 점검, 발표 중 배포 서버 운영 |

## 서비스 주요 기능

- 사용자 조건 입력: 희망 지역, 소득, 자산, 부채, 청약통장, 월 저축 가능액, 희망 면적과 가격대를 저장합니다.
- 청약 추천: 실제 공고와 보강 데이터를 바탕으로 자격, 자금, 지역, 일정 점수를 계산해 추천 후보를 제시합니다.
- 공고 상세 확인: 청약 공고의 공급 정보, 주택형 옵션, 공식 PDF 분석 상태, 공식 근거 확인 항목을 보여줍니다.
- 자금 로드맵: 선택한 주택형 기준으로 계약금, 중도금, 잔금, 융자금, 부족액, 월 준비 목표를 계산합니다.
- AI 코치: 선택 공고와 주택형, 사용자 조건, 공식 근거를 바탕으로 다음 행동과 확인 포인트를 정리합니다.
- 청약 지도: 공고 위치를 지도에서 확인하고 주변 부동산, 은행, 길찾기 흐름을 제공합니다.
- 금융상품: 예적금 상품의 기간별 금리 옵션을 비교하고 자금 준비 목적에 맞는 상품을 가입 후보로 저장합니다.
- 경제 NOW: 부동산, 환율, 금, 시장 지표 등 주거 의사결정에 참고할 수 있는 데이터를 시각화합니다.
- 청약 아고라: 청약 관련 영상 검색과 커뮤니티 게시글/댓글 기능을 제공합니다.
- MY PAGE: 관심 공고, 관심 주택형 옵션, 가입 금융상품, 프로필 정보를 한곳에서 확인합니다.

## 금융상품 추천 알고리즘 설명

금융상품 추천은 단순 최고 금리순이 아니라 사용자의 청약 자금 준비 상황에 맞는 상품을 우선합니다. 핵심 기준은 준비 기간, 월 납입 여력, 금리, 상품 유형, 옵션 다양성, 공식 정보 신뢰도입니다.

1. 준비 기간 적합도
   - 사용자의 목표 준비 기간과 상품 기간이 가까울수록 높은 점수를 부여합니다.
   - 정확히 일치하면 가장 높고, 3개월, 6개월, 12개월, 18개월 차이 순으로 점수가 낮아집니다.

2. 월 납입 가능성
   - 사용자의 월 저축 가능액과 상품 월 납입 한도를 비교합니다.
   - 월 납입 한도가 사용자의 저축 여력과 잘 맞을수록 높은 점수를 부여합니다.

3. 금리 점수
   - 기본 금리와 우대 금리 중 활용 가능한 최고 금리를 기준으로 계산합니다.
   - 금리가 높을수록 유리하지만, 금리만으로 추천이 결정되지 않도록 다른 조건과 함께 반영합니다.

4. 상품 유형과 목적성
   - 청약, 주거, 내집 마련, 청년, 적금 계열처럼 자금 준비 목적과 맞는 상품을 우선합니다.
   - 예금자 보호 여부와 공식 출처 URL도 신뢰도 신호로 반영합니다.

5. 옵션 다양성
   - 같은 상품 안에 여러 기간별 금리 옵션이 있으면 비교 가능성이 높다고 보고 일부 가점을 부여합니다.

최종 점수는 0~100점 사이로 제한하며, 금융상품 목록과 자금 로드맵 추천 상품 영역에서 사용합니다. 상세 설계는 `docs/submit/docs_submit/recommendation_algorithm_detail.txt`에 정리되어 있습니다.

## 생성형 AI 활용 내용

이 프로젝트는 SSAFY GMS OpenAI 호환 API를 통해 생성형 AI 기능을 사용합니다.

- AI 코치 요약: 선택한 청약 공고, 주택형 옵션, 자금 로드맵, 공식 PDF 근거, 사용자 조건을 묶어 다음 행동을 요약합니다.
- 전체 화면 챗봇: 현재 사용자가 보고 있는 화면 맥락을 page_context로 전달해 화면별 질문 추천과 답변을 제공합니다.
- 공식 PDF 보조 분석: 규칙 기반 추출로 부족한 부분이 있을 때 LLM을 보조적으로 사용해 체크리스트와 설명을 강화합니다.
- 안전 필터: 청약 당첨 보장, 대출 가능 보장, 허위 확정 표현처럼 위험한 답변은 제한하고 공식 공고문 확인을 우선 안내합니다.
- fallback 구조: API 키 오류나 외부 AI 장애 상황에서도 규칙 기반/template 답변으로 시연 흐름이 완전히 끊기지 않도록 설계했습니다.

## 프로젝트 후기 및 느낀 점

이번 프로젝트는 청약이라는 현실적인 의사결정 문제를 데이터, 규칙, UI, AI로 연결하는 과정이었습니다. 단순히 공고를 보여주는 것보다 사용자가 실제로 궁금해하는 것은 "내 조건에 맞는지", "돈은 얼마나 부족한지", "무엇을 먼저 확인해야 하는지"라는 점을 확인했습니다.
...
기존 청약 추천 흐름은 유지하고, 금융 상품 비교 애플리케이션 명세는 `금융 광장`과 `MY PAGE`, 지도 확장으로 통합했습니다.

| 요구사항 | 구현 위치 |
|---|---|
| F1301 메인 페이지 | 대시보드 금융 광장 카드, 사이드바 `금융 광장` 그룹 |
| F1302 회원 커스터마이징 | `profiles.CustomUser`, 회원가입/로그인/로그아웃, 세션 병합 |
| F1303-1 예적금 데이터 저장 | `FinancialProduct`, `FinancialProductOption`, `import_finlife_products` |
| F1303-2 예적금 목록 조회 | `/api/products`, `/finance/products` 은행/카테고리/기간/검색/정렬 |
| F1303-3 예적금 상세 조회 | `/api/products/<id>`, `/finance/products/:productId`, 로그인 사용자 가입 저장 |
| F1304 현물 상품 시각화 | `/api/market/assets`, `/finance/economy-now` 금·은 기간 차트 |
| F1305-1 관심 종목 검색 | `/api/videos/search`, `/finance/agora` YouTube 검색 |
| F1305-2 관심 종목 상세 보기 | `/finance/agora` 영상 모달 iframe 재생 |
| F1306-1 근처 은행 검색 | `/api/places/search`, `/map` 은행 탭 |
| F1306-2 경로 안내 기능 | `/map` Kakao Map 길찾기 URL, `/api/places/route` |
| F1307 커뮤니티 기능 | `/api/agora/posts`, `/api/agora/comments`, `/finance/agora` 게시글/댓글 CRUD |
| F1308 프로필 페이지 | `/my-page` 프로필, 관심목록, 가입상품, 금리 그래프 |
...
화면 시연 전에 실제 데이터를 먼저 DB에 수집하고 가공합니다. 이 프로젝트의 화면은 브라우저가 외부 API를 직접 호출하지 않고, 운영자가 백엔드 관리 명령으로 수집한 DB 데이터를 조회합니다.
...
KOREAEXIM_API_KEY=한국수출입은행_환율_AUTHKEY
METALS_DEV_API_KEY=Metals.dev_API_KEY
BOK_ECOS_API_KEY=한국은행_ECOS_API_KEY
KRX_API_KEY=KRX_Open_API_AUTH_KEY
```
...
기본 수집은 50개씩 4페이지, 약 200개 정책 후보를 가져옵니다. 수집 후 서비스 내부 matcher가 청약, 주거, 대출, 자금 관련 정책과 사용자 조건을 기준으로 다시 걸러냅니다.
...
기본 수집은 페이지당 250개씩 7페이지를 조회하고, 서비스 대상 공고에 대해 LH 공급정보 상세 API로 주택형, 면적, 일부 분양가를 보강합니다. 기존 공고를 지우고 다시 수집해야 할 때만 `python manage.py import_lh --clear`를 사용합니다.
...
LH 공고 수집 후 공식 PDF 분석을 실행하면 상세 화면과 자금 로드맵에서 PDF 기반 주택형 옵션, 계약금, 중도금, 잔금, 융자금 일정이 우선 사용됩니다. 이미 분석된 공고까지 다시 분석하려면 `--force`를 사용합니다.
...
좌표가 비어 있는 공고를 대상으로 Kakao Local REST API 좌표 보강을 시도합니다. 이미 좌표가 있는 공고까지 다시 보정해야 할 때만 `--overwrite`를 사용합니다.

### 7.6 Economy NOW 압축 수집

경제 NOW는 금, 은, 환율, 기준금리, KOSPI/KOSDAQ, 부동산 거래 요약을 운영자 명령 한 번으로 DB에 저장합니다.
...
기본값은 무료 API 사용량을 아끼기 위해 최근 30일 범위만 수집합니다. 발표용 히스토리를 넓히고 싶을 때만 `--days`를 늘립니다.

```bash
python manage.py import_market_data --days 180
python manage.py import_market_data --asset silver --days 30
python manage.py import_market_data --asset exchange
python manage.py import_market_data --asset krx
```

수집 대상:

- `gold`: 공공데이터포털 금융위원회 일반상품시세정보, KRX 금시장 종가
- `silver`: Metals.dev 은 가격, 무료 플랜 보호를 위해 30일 단위 분할 수집
- `exchange`: 한국수출입은행 환율 정보, 신규 도메인 `oapi.koreaexim.go.kr`
- `ecos`: 한국은행 ECOS 기준금리
...
http://localhost:8000/api/market/assets?asset=gold
http://localhost:8000/api/market/assets?asset=silver
```

fixture는 발표 화면을 비우지 않기 위한 보조 데이터입니다. 실제 데이터가 충분한 영역은 DB에 저장된 실제 수집값이 먼저 사용됩니다.
...
발표 직전에는 주요 API를 한 번씩 미리 호출해 캐시를 데웁니다. 첫 발표 관객이나 심사위원이 접속했을 때 추천 계산, 금융상품 추천, 시장 데이터 조회, AI 챗봇 첫 응답이 한꺼번에 시작되지 않도록 하는 절차입니다. `/api/videos/default`는 YouTube 검색 API를 호출하지 않는 정적 추천 영상 응답이므로, 이 단계에서는 청약 아고라 영상 영역이 정상 응답하는지만 확인합니다.
