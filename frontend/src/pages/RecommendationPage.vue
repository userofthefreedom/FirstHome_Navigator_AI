...
<template>
  <div class="space-y-5">
    ...
    <section v-else class="space-y-4">
      ...
      <article v-for="(item, index) in sortedRecommendations" :key="item.notice_id" class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <div class="grid items-start gap-4 xl:grid-cols-[minmax(0,1fr)_240px] 2xl:grid-cols-[minmax(0,1fr)_260px]">
          <div>
          ...
          <div class="rounded-lg border border-slate-200 bg-slate-50 p-4">
            <div class="mb-3 flex items-center justify-between gap-2">
              <p class="inline-flex items-center gap-1 rounded-md bg-white px-2 py-1 text-xs font-bold text-slate-600">
                <ArrowDown class="h-3.5 w-3.5" />
                {{ sortLabel() }}
              </p>
              <button
                type="button"
                class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 bg-white text-slate-600 transition hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-60"
                :title="isFavorite(item.notice_id) ? '공고 저장됨' : '공고 저장'"
            ...
            <RouterLink
              ...
              class="mt-4 inline-flex h-10 w-full items-center justify-center gap-2 rounded-lg bg-blue-600 text-sm font-bold text-white transition hover:bg-blue-700"
            >
              공식 근거 보기
